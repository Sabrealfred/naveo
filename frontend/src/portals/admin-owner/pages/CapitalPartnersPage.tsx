...
import { Card, Col, Row, Tabs, Tag, Table, Statistic, Space, Typography, Input, Segmented, Progress, Button, Badge, Divider, Alert, Timeline } from 'antd';
...
const CapitalPartnersPage = () => {
  ...
  const assetFocus = capitalPartners.reduce((acc, partner) => {
    partner.focusAssets.forEach((asset) => {
            ...
            children: (
              <Row gutter={[16, 16]}>
                {capitalPartners.map((partner) => (
                  <Col xs={24} md={12} lg={8} key={`${partner.name}-ltv`}>
                    <Card size="small" style={{ background: '#fafafa' }}>
                      <Space direction="vertical" style={{ width: '100%' }}>
                        ...
                        <div style={{ fontSize: '13px' }}>
                          ...
                          <div style={{ marginBottom: 4 }}>
                            <Text type="secondary">LTV:</Text> <Tag color="green">{partner.ltvRange || 'Negociación'}</Tag>
                          </div>
                          <div style={{ marginBottom: 4 }}>
                            <Text type="secondary">Ticket:</Text> <Text>{partner.ticketSize}</Text>
                          </div>
